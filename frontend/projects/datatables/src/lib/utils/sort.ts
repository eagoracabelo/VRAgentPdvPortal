...
export function sortRows(rows: any[], columns: any[], dirs: any): any[] {
  ...
  const cachedDir = {
    prop: dirs.column,
    dir: dirs.dir,
    valueGetter: getterForProp(dirs.column),
    compareFn: cols[dirs.column] || orderByComparator,
  };
  ...
  return temp.sort(function (rowA: any, rowB: any) {
    ...
    const rowAIndex = rowToIndexMap.get(rowA);
    const rowBIndex = rowToIndexMap.get(rowB);

    if (!rowAIndex || !rowBIndex) return 0;
