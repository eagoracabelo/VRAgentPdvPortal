...
export class TableBodyComponent implements OnDestroy {
  ...
  /**
   * Creates an instance of TableBodyComponent.
   */
   ...
   * manipulated.   As an example, if the height of row 0 is 30 rem and row 1 is
   * 100 rem then following styles are generated:
   *
   * transform: translate3d(0rem, 0rem, 0rem);    ->  row0
   * transform: translate3d(0rem, 30rem, 0rem);   ->  row1
   * transform: translate3d(0rem, 130rem, 0rem);  ->  row2
   *
  ...
  getBottomSummaryRowStyles(): any {
    if (!this.scrollbarV || !this.rows?.length) {
      return null;
    }
  ...
  refreshRowHeightCache(): void {
    ...
    if (this.rows?.length) {
      ...
      this.rowHeightsCache.initCache({
        ...
        externalVirtual: this.scrollbarV,
