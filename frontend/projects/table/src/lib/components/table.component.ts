...
import { IVrcTableConfig } from '../vrc-table.module';
import { TableBodyComponent } from './body/body.component';
import { TableGroupHeaderDirective } from './body/directives/body-group-header.directive';
import { TableColumnDirective } from './columns/column.directive';
import { TableFooterDirective } from './footer/directives/footer.directive';
import { TableHeaderComponent } from './header/header.component';
import { TableRowDetailDirective } from './row-detail/row-detail.directive';
...
{
  @Input() exportFile: boolean = true;

  /**
   * Set translation strings for table component
   */
  @Input()
  set translation(lang: string) {
    this._translation = lang;
    this._translationService.loadTranslation(lang).subscribe();
  }
  ...
  get offset(): number {
    return Math.max(
      Math.min(this._offset, Math.ceil(this.rowCount / this.pageSize) - 0.0625),
      0,
    );
  }
  ...
  @Input() cssClasses: any = {
    sortAscending: 'table-icon-up',
    sortDescending: 'table-icon-down',
    sortUnset: 'table-icon-sort-unset',
  };
  ...
  rowDetail!: TableRowDetailDirective;

  /**
   * Group Header templates gathered from the ContentChild
   */
  @ContentChild(TableGroupHeaderDirective)
  groupHeader!: TableGroupHeaderDirective;

  /**
   * Footer template gathered from the ContentChild
   */
  @ContentChild(TableFooterDirective)
  footer!: TableFooterDirective;
  ...
  constructor(
    ...
    private readonly configuration: IVrcTableConfig,
  ) {
  ...
  ngOnInit(): void {
    if (!this._translation) {
      this._translationService.loadTranslation('pt-BR').subscribe();
    }
  ...
  recalculateDims(): void {
    ...
    if (this.scrollbarV) {
      const pxHeight = this.dimensionsHelper.getHeight(dimensions)(
        this.headerHeight,
        this.footerHeight,
      );
      this.bodyHeight = pxHeight / htmlFontSize;
    }
  ...
  /**
   * listen for changes to input bindings of all TableColumnDirective and
